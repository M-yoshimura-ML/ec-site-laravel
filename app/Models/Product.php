...
class Product extends Model
{
    ...
    public function scopeSortOrder($query, $sortOrder){
        if($sortOrder === null || $sortOrder === \App\Constants\Common::SORT_ORDER['recommend']){
            return $query->orderBy('sort_order', 'asc');
        }
        if($sortOrder === \App\Constants\Common::SORT_ORDER['higherPrice']){
            return $query->orderBy('price', 'desc');
        }
        if($sortOrder === \App\Constants\Common::SORT_ORDER['lowerPrice']){
            return $query->orderBy('price', 'asc');
        }
        if($sortOrder === \App\Constants\Common::SORT_ORDER['later']){
            return $query->orderBy('products.created_at', 'desc');
        }
        if($sortOrder === \App\Constants\Common::SORT_ORDER['older']){
